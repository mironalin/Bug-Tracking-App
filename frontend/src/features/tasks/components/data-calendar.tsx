...
const CustomToolbar = ({ date, onNavigate }: CustomToolbarProps) => {
  return (
    <div className="flex mb-4 gap-x-2 items-center w-full lg:w-auto lg:justify-start justify-center">
...
export const DataCalendar = ({ data }: DataCalendarProps) => {
  ...
  return (
    <Calendar
      ...
      formats={{ weekdayFormat: (date, culture, localizer) => localizer?.format(date, "EEE", culture) ?? "" }}
      components={{
        eventWrapper: ({ event }) => (
          <EventCard
            slug={event.slug}
